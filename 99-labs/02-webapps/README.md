...
Make sure that `kubectl` is installed locally and carry out some useful customization steps:
...
- `kubectl exec <name> -it  -- <command>`: execute the shell `<command>` on the pod called `<name>` (works only if the container image has a shell, containers built from Go programs usually don't have one unless explicitly added during the build);
...
We could have used the declarative style and deploy the pod from a YAML manifest instead; the below would be roughly equivalent to the above `kubectl run` command:
