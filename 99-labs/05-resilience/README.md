...
> :bulb: Tip
> 
> You can browse the full documentation of the `resilient` package: start a local godoc server in the root directory of the `resililient` package with `godoc -http=:6060` and the open [`http://localhost:6060/pkg/resilient`](http://localhost:6060/pkg/resilient) in your browser.
...
1. Create a `defaultBackoff` retry policy that will allow at most 6 retries, using the base time of 250 ms and 2 sec timeout.
